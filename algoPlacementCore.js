...
const AlgoPlacement = (function () {
  ...
  const BIG_COURTS = ["Multi Sport Court", "Basketball Court", "Handball", "Volleyball"];

  // name, long side (m), short side (m), colour -> FINAL envelopes (run-off included)
  const SPORTS = [
    { name: "Multi Sport Court", long: 20.0, short: 12.0, color: [31, 119, 180] },
    { name: "Basketball Court", long: 22.0, short: 13.0, color: [255, 127, 14] },
    { name: "Badminton", long: 13.4, short: 6.1, color: [44, 160, 44] },
    { name: "Yoga", long: 10.0, short: 5.0, color: [148, 103, 189] },
    { name: "Bocce Court", long: 18.0, short: 3.0, color: [140, 86, 75] },
    { name: "Handball", long: 30.0, short: 16.0, color: [227, 119, 194] },
    { name: "Volleyball", long: 16.0, short: 8.0, color: [23, 190, 207] },
    { name: "Calisthenics", long: 8.0, short: 6.0, color: [120, 120, 40] },
    { name: "Ping Pong", long: 5.7, short: 3.5, color: [255, 214, 0] },
    { name: "Mini Golf", long: 12.0, short: 1.5, color: [102, 194, 165] },
    { name: "Sandpit", long: 4.0, short: 4.0, color: [222, 184, 135] }
  ];
   ...
   */
  function makeSite({ foot, setback, entries, keepClear }) {
  ...
  function placeCourt(L, name, w0, h0, rng, noise, remaining) {
    ...
    const directs = [], others = [];
    let anySpot = false;
    for (const [w, h] of orients) {
      const spots = genSpots(L, w, h, big);
  ...
  function runAttempt(grid, W, items, ring, rng, noise, deadline, pickK, edgeFirst, bigSet) {
    const L = new Layout(grid, W, bigSet);
    L.pickK = pickK;
  ...
  function validate(grid, W, minAcc, courts, paths, sec, bigSet) {
    const issues = [];
    const gap = toCells(COURT_GAP_M);
    ...
    for (let i = 0; i < courts.length; i++) {
      for (let j = i + 1; j < courts.length; j++) {
        ...
        } else if (overlap(inflate(cr[i], gap), cr[j])) issues.push(courts[i][0] + " is closer than " + COURT_GAP_M.toFixed(1) + " m to " + courts[j][0]);
      }
   ...
   */
  async function planOnce(site, requests, pathWm, ring, timeLimit, seed, shuffle, variant, seen, edgeFirst, leftoverPath, bigSet, opts) {
    const o = opts || {};
    const t0 = now();
    ...
    for (;;) {
      ...
      const first = attempts === 0 && !shuffle;
      // big courts first (hardest to fit), then by area; past failures jump the queue
      const keyed = items.map(it => {
        const jit = first ? 1.0 : rng.uniform(loJ, hiJ);
        return [[bigSet.has(it[0]) ? 0 : 1, -it[1] * it[2] * jit * (1.0 + 0.5 * (boost[it[0]] || 0))], it];
      });
      keyed.sort((a, b) => cmp(a[0], b[0]));
      const order = keyed.map(k => k[1]);
      const [noise, pickK] = shuffle ? [6.0, 5] : first ? [0.0, 1] : [1.5, 1];
      const [L, placed, unplaced, warns] = runAttempt(grid, W, order, ring, rng, noise, hard, pickK, edgeFirst, bigSet);
      for (const u of unplaced) boost[u[0]] = (boost[u[0]] || 0) + 1;
      const onEdge = L.courts.filter(([, r]) => grid.edgeContact(r) >= emin).length;
      // best = most court area, then most courts on the setback line, then least path
      const score = [placed.reduce((s, p) => s + p[1] * p[2], 0), onEdge, -rectUnionArea(L.paths), placed.length];
    ...
    if (shuffle) {
      ...
      const topEdge = Math.max(...pool.map(r => r.score[1]));
      pool = pool.filter(r => r.score[1] >= topEdge - 1);
      pool.sort((a, b) => -a.score[2] - -b.score[2]);
    ...
    const issues = validate(grid, W, L.minAcc, L.courts, L.paths, sec, bigSet);
    const presets = new Map(SPORTS.map(s => [s.name, s]));
    const courtsOut = L.courts.map(([name, r]) => {
      const p = presets.get(name);
      const rw = (r[2] - r[0]) * RES;
      return { name, rect: grid.toM(r), long: p.long, short: p.short, rotated: p.long !== p.short && Math.abs(rw - p.short) < 1e-6, onEdge: grid.edgeContact(r) >= emin, big: bigSet.has(name) };
    });
    return {
      courts: courtsOut,
   ...
   *   settings: { pathW, minPathW, ring, timeLimit, seed, shuffle, variant, seen, edgeFirst, leftoverPath, strictGap }
   */
  async function planLayout(site, requests, settings, opts) {
    const s = Object.assign({ pathW: DEFAULT_PATH_W, minPathW: MIN_PATH_W_M, ring: false, timeLimit: DEFAULT_TIME, seed: 1, shuffle: false, variant: 0, seen: null, edgeFirst: true, leftoverPath: false, strictGap: false }, settings || {});
    ...
    for (let k = 0; k < widths.length; k++) {
      ...
      const plan = await planOnce(site, requests, widths[k], s.ring, tl, s.seed, s.shuffle, s.variant, s.seen, s.edgeFirst, s.leftoverPath, bigSet, opts);
  ...
  function fitCheck(site, pathWm, ring, strictGap) {
    const grid = site.grid;
    const bigSet = strictGap ? new Set() : new Set(BIG_COURTS);
    const W = toCells(pathWm);
    const L0 = new Layout(grid, W, bigSet);                       // empty roof
    const L1 = new Layout(grid, W, bigSet);                       // lift landings + main corridor only
    const notes = [];
    const rng = makeRng(1);
  ...
  function buildReport(plan, site) {
    ...
    const lines = [
      ...
      "Courts touching the setback line: " + s.onEdge + " of " + s.placed + "   |   Big courts (one path side only): " + s.bigN,
      "Main pathway " + s.pathW.toFixed(1) + " m wide" + narrowed,
      "Primary paths: " + s.pathArea.toFixed(0) + " m² (" + (100 * s.pathArea / u).toFixed(0) + "%)   Secondary " + COURT_GAP_M.toFixed(1) + " m paths: " + s.secArea.toFixed(0) + " m² (" + (100 * s.secArea / u).toFixed(0) + "%)",
      garden,
      "Sports area: " + u.toFixed(0) + " m² (bounding box " + bw + " x " + bh + " m)   |   " + s.attempts + " attempts in " + s.elapsed.toFixed(1) + " s"
    ];
    if (plan.unplaced.length) lines.push("NOT placed: " + plan.unplaced.map(x => x.name + " (" + x.reason + ")").join("; "));
    plan.warnings.forEach(w => lines.push("Warning: " + w));
    lines.push(plan.issues.length ? "CHECK FAILED: " + plan.issues.join("; ")
      : "Checks passed: " + COURT_GAP_M.toFixed(1) + " m path around normal courts and lift/ramp sides, big courts touch a path on one side, pathways connected.");
    return lines.join("\n");
  }

  return {
    RES, SPORTS, BIG_COURTS, COLOR_VC, COLOR_PATH, COLOR_GARDEN,
