...
check("unknown fields are dropped, so nothing else can ride along into the settings file", !("evil" in core.normalizeProfile({ view: "simple", evil: "<script>" })) && Object.keys(core.normalizeProfile(full)).sort().join() === "name,person,quiz,role,schema,theme,updated,view" && Object.keys(core.normalizeProfile(full).person).sort().join() === "name,photo");
...
check("Simple draws the rail as Site | Sport Garden Zones Plants Furniture Combine | Analysis (no separator at an end or twice in a row)", simpleRail === "modeSite | modeSport modeGardenBlocks btn-zone-toggle btn-vegetation-toggle btn-furniture-toggle modeCombine | modeAnalysis", simpleRail);
...
check("the tab's content, its buttons, the name and photo fields and the welcome screen's line exist", ["profile-content", "profile-view", "profile-view-note", "profile-status", "btn-profile-reload", "btn-profile-export", "btn-profile-import", "btn-profile-reset", "profile-import-file", "sessionGateProfile", "sessionGatePhoto", "profile-avatar", "profile-person-name", "btn-profile-photo", "btn-profile-photo-remove", "profile-photo-file"].every(id => html.includes('id="' + id + '"')));
...
function harness({ stored = null, elements = {} } = {}) {
  ...
  const rev = { up: true, profile: null, posts: [], gets: 0, refuse: false };
  ...
  const sandbox = {
    ...
    escapeHtml: s => String(s), showToast: (t, m) => calls.toasts.push(t + ": " + m), triggerDownload: (blob, name) => calls.downloads.push(name),
