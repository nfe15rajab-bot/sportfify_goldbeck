...
})());
{
  ...
  check("in its place: 'What's new' with five cards: Profile, the quiz, manual placement, algorithmic placement and the results, each with a button", /What's new/.test(overview) && features.join() === "profile,quiz,manual,algo,results" && /class="feature-head"[^>]*><i class="ti ti-user-circle"/.test(overview) && (overview.match(/class="feature-card"/g) || []).length === 5);
  const mainJs = read("main.js");
  check("each card's button is wired in main.js: the profile tab, the quiz, the Results tab, and Combine in the manual or the algorithmic mode (algoSetMode)", /feature === "profile"\) setMode\("profile"\)/.test(mainJs) && /feature === "results"\) setMode\("analysis"\)/.test(mainJs) && /feature === "quiz"/.test(mainJs) && /feature === "manual" \|\| feature === "algo"/.test(mainJs) && /algoSetMode\(feature\)/.test(mainJs) && /function algoSetMode\(mode\)/.test(read("algoPlacementUI.js")) && /mode === "algo" \? "algo" : "manual"/.test(read("algoPlacementUI.js")));
